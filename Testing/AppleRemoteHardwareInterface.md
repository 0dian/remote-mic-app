# Apple Remote 通用硬件接口与 A2540/A2854 实机测试手册

## 适用范围

- 日期：2026-09-05。
- 目标硬件：Siri Remote A2540（Lightning）与 A2854（USB-C）。其他代际不在本手册兼容范围内。
...
1. 确认遥控器型号为 A2540（Lightning 版）或 A2854（USB-C 版），并记录本轮型号。
...
预期结果：连接状态显示“Apple Siri Remote 已连接”，遥控器出现在设备选择区并显示实际型号。A2540 与 A2854 使用同一张产品图片、比例和锚点。失败判定：授权并重启后仍只显示正在查找、型号错误、两型号布局不一致，或者按键会操作系统但 App 没有任何响应。
...
5. 观察鼠标右下角的指示器；把鼠标移到屏幕四角后重复移动与滚动。

预期结果：指针移动稳定且相较旧包明显更快、轻触只点击一次、外圈可以连续滚动且步幅足够；外圈向中心的径向轨迹锁定为鼠标移动，累计圆周轨迹达到阈值后锁定为滚动，同一次接触不会在两种模式之间切换。移动和滚动指示器固定优先显示在鼠标右下方，与鼠标本体保留间距；屏幕边缘可自动换到其他不遮挡位置。触摸结束后不漂移。

失败判定：完全没有触摸回调、指示器未出现或覆盖鼠标、App 退出、点击重复、径向滑动触发滚动、圆周滑动移动鼠标、同一次接触反复切换模式、滚动方向相反、快速滚动失控、移动或滚动仍明显迟缓，或实体按压附带鼠标动作。
...
| 设备组合 | 仅 A2540、仅 A2854、Siri Remote + Magic Mouse、Siri Remote + RC001/RC003、两只同型号设备（条件允许时） | 同名控制项按设备实例隔离；普通鼠标性能不变 |
...
6. 检查移动/滚动指示器位于鼠标右下且不重叠，并覆盖屏幕四角的自动换侧。

预期结果：指针稳定，无明显静止漂移；点击只触发一次；实体方向环或中心按压不附带移动或点击；径向轨迹锁定鼠标、圆周轨迹锁定滚动且抖动不切换；指示器可见但不覆盖系统鼠标；滚动方向一致且低速可精确控制、快速不突跳。触摸结束和断连不会留下输入状态。
